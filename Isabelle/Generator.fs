...
"""
    ...
    let mostGeneralUnifier a b ls:string = 
        let rec substitution = function
            | (v,f)::ss -> 
                """if x = """ + v + """ then """ + f + """ else """ + substitution ss
            | [] -> ""
        let uuu ls = 
            let uuuSingle v f n = "sub" + string n + ": ‹u " + v + " = " + f + "› "
...
qed""")
    ...
    let rec genClause = function 
        | x::[] -> genAtom x
        | x::xs -> genAtom x + "," + genClause xs 
        | [] -> ""

    let genFormula (f:ClausalFormula) =
        let rec genFormula' = function 
            | x::[] -> "{" + genClause x + "}"
    ...
    let rec defineAtoms (al:Atom list):string =
        let rec defineArgs (al':argument list) =
            ...
            | x::xs -> 
                ...
                | _ -> ", " + defineArgs xs)
            | [] -> ""
        let defineAtom (i, al', b) = 
            "definition " + genAtom (i, al', b) + " :: ‹fterm literal› where \n  ‹" + 
            genAtom (i, al', b) + " = " + 
            (match b with 
            | true ->  "Pos" 
            | false -> "Neg") + 
        ...
        | [] -> ""
        | (i, al', b)::xs -> 
            "\n\n" + defineAtom (i, al', b) + defineAtoms xs

    let rec allResolvedClauses = function 
        | Copy(_)::xs -> allResolvedClauses xs
        | Resolve(_,_,x,_,_)::xs -> x::allResolvedClauses xs
        | Rename(_,x,_,_)::xs -> x::allResolvedClauses xs
        | [] -> []

    let genDerivation ((f, al):Proof):string = 
        "\n‹resolution_deriv " + genFormula f + 
        "\n                  " + genFormula (f @ allResolvedClauses al) + "›"
    ...
    type unifier = 
        | E
        | U of Atom * Atom * ((argument * argument) Set)
